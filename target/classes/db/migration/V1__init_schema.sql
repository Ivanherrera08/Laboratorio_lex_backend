...
-- SCRIPT: Data Definition Language (DDL) — PostgreSQL (Flyway Migration V1)
-- DESCRIPCIÓN: Script de creación de esquema, restricciones, índices, triggers de
--              inmutabilidad y datos semilla (F-01 a F-35 y NF-01 a NF-12).
-- MOTOR: PostgreSQL 14+
-- =====================================================================================
...
CREATE TABLE IF NOT EXISTS usuarios (
    ...
    intentos_fallidos   SMALLINT NOT NULL DEFAULT 0
                            CHECK (intentos_fallidos >= 0 AND intentos_fallidos <= 3),
...
CREATE INDEX IF NOT EXISTS idx_tokens_recuperacion_vigentes ON tokens_recuperacion(expiracion) WHERE usado = FALSE;
...
DROP TRIGGER IF EXISTS trg_areas_restringidas_updated_at ON areas_restringidas;
...
CREATE TABLE IF NOT EXISTS empleados (
    ...
    telefono                VARCHAR(20),
    departamento_id         INTEGER NOT NULL REFERENCES departamentos(id) ON DELETE RESTRICT,
    codigo_tarjeta_rfid     VARCHAR(50) UNIQUE,
...
-- F-16: función y trigger de inmutabilidad de documento
...
DROP TRIGGER IF EXISTS trg_empleados_doc_inmutable ON empleados;
CREATE TRIGGER trg_empleados_doc_inmutable
    BEFORE UPDATE ON empleados
    FOR EACH ROW EXECUTE FUNCTION zone_control.fn_empleados_bloquear_doc_inmutable();

CREATE UNIQUE INDEX IF NOT EXISTS idx_empleados_numero_documento ON empleados(numero_documento);
CREATE UNIQUE INDEX IF NOT EXISTS idx_empleados_codigo_tarjeta_rfid ON empleados(codigo_tarjeta_rfid) WHERE codigo_tarjeta_rfid IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_empleados_nombres_apellidos ON empleados(apellidos, nombres);
CREATE INDEX IF NOT EXISTS idx_empleados_departamento ON empleados(departamento_id);

-- =====================================================================================
-- 10. TABLA: autorizaciones_zona (F-21)
-- =====================================================================================
CREATE TABLE IF NOT EXISTS autorizaciones_zona (
    id                  BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_autorizaciones_empleado ON autorizaciones_zona(empleado_id);
CREATE INDEX IF NOT EXISTS idx_autorizaciones_area ON autorizaciones_zona(area_id);
CREATE INDEX IF NOT EXISTS idx_autorizaciones_activas ON autorizaciones_zona(empleado_id, area_id) WHERE activo = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_historial_timestamp_resultado ON historial_accesos("timestamp", resultado_acceso);
CREATE INDEX IF NOT EXISTS idx_historial_empleado ON historial_accesos(empleado_id);
CREATE INDEX IF NOT EXISTS idx_historial_area ON historial_accesos(area_id);
CREATE INDEX IF NOT EXISTS idx_historial_documento_ingresado ON historial_accesos(numero_documento_ingresado);

DROP TRIGGER IF EXISTS trg_historial_accesos_no_update ON historial_accesos;
...
DROP TRIGGER IF EXISTS trg_historial_accesos_no_delete ON historial_accesos;
...
CREATE INDEX IF NOT EXISTS idx_bitacora_timestamp ON bitacora_auditoria("timestamp");
CREATE INDEX IF NOT EXISTS idx_bitacora_usuario ON bitacora_auditoria(usuario_id);
CREATE INDEX IF NOT EXISTS idx_bitacora_tipo_operacion ON bitacora_auditoria(tipo_operacion);
CREATE INDEX IF NOT EXISTS idx_bitacora_modulo_tabla ON bitacora_auditoria(modulo_tabla);
CREATE INDEX IF NOT EXISTS idx_bitacora_valor_anterior_gin ON bitacora_auditoria USING GIN (valor_anterior);
CREATE INDEX IF NOT EXISTS idx_bitacora_valor_nuevo_gin ON bitacora_auditoria USING GIN (valor_nuevo);

DROP TRIGGER IF EXISTS trg_bitacora_auditoria_no_update ON bitacora_auditoria;
CREATE TRIGGER trg_bitacora_auditoria_no_update
    BEFORE UPDATE ON bitacora_auditoria
    FOR EACH ROW EXECUTE FUNCTION zone_control.fn_prevent_update_delete();

DROP TRIGGER IF EXISTS trg_bitacora_auditoria_no_delete ON bitacora_auditoria;
...
-- =====================================================================================
CREATE TABLE IF NOT EXISTS registro_sincronizacion_socio (
    id                          BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_sincronizacion_estado ON registro_sincronizacion_socio(estado);
CREATE INDEX IF NOT EXISTS idx_sincronizacion_departamento ON registro_sincronizacion_socio(departamento_id);
CREATE INDEX IF NOT EXISTS idx_sincronizacion_periodo ON registro_sincronizacion_socio(periodo_inicio, periodo_fin);
CREATE INDEX IF NOT EXISTS idx_sincronizacion_pendientes ON registro_sincronizacion_socio(fecha_proximo_reintento)
    WHERE estado = 'EN_REINTENTO';
CREATE INDEX IF NOT EXISTS idx_sincronizacion_payload_gin ON registro_sincronizacion_socio USING GIN (payload_json);
